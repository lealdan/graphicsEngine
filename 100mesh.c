...
double vary[renVARYDIMBOUND * renVERTNUMBOUND];
double varyA[renVARYDIMBOUND];
double varyB[renVARYDIMBOUND];
double varyC[renVARYDIMBOUND];
...
void meshRender(meshMesh *mesh, renRenderer *ren, double unif[],
	...
	if (ren->attrDim == mesh->attrDim) {
		...
		for (int i = 0; i < mesh->triNum; i++) {
			...
			double *a = meshGetVertexPointer(mesh, triangle_i[0]);
			double *b = meshGetVertexPointer(mesh, triangle_i[1]);
			double *c = meshGetVertexPointer(mesh, triangle_i[2]);

            //Transform the textures
            ren->transformVertex(ren, unif, a, varyA);
            ren->transformVertex(ren, unif, b, varyB);
            ren->transformVertex(ren, unif, c, varyC);
...
// 			ren->transformVertex(ren, unif, &mesh->vert[i * ren->attrDim], &vary[i * ren->varyDim]);
// 		}
//
