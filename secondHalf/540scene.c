...
void sceneRender(sceneNode *node, GLdouble parent[4][4], GLint modelingLoc,
	...
	if (node == NULL) {
		...
		for (int i = 0; i < unifNum; i++) {
			if (unifDims[i] == 1) {
				GLfloat vec[1];
				vec[0] = node->unif[count];
				glUniform1fv(unifLocs[i], 1, vec);
				count += 1;
			}
			else if (unifDims[i] == 2) {
				GLfloat vec[2];
				vec[0] = node->unif[count];
				vec[1] = node->unif[count + 1];
				glUniform2fv(unifLocs[i], 1, vec);
				count += 2;
			}
				...
				vec[0] = node->unif[count];
				vec[1] = node->unif[count + 1];
				vec[2] = node->unif[count + 2];
				glUniform3fv(unifLocs[i], 1, vec);
				count += 3;
			}
				...
				vec[1] = node->unif[count + 1];
				vec[2] = node->unif[count + 2];
				vec[3] = node->unif[count + 3];
				glUniform4fv(unifLocs[i], 1, vec);
				count += 4;
			}
		}

		/* Set the node's textures */
		for (int j = 0; j < node->texNum; j++) {
			switch(j) {
				case 0:
					texRender(node->tex[j], GL_TEXTURE0, j, textureLocs[j]);
					break;
				case 1:
					texRender(node->tex[j], GL_TEXTURE1, j, textureLocs[j]);
					break;
				case 2:
					texRender(node->tex[j], GL_TEXTURE2, j, textureLocs[j]);
					break;
				case 3:
					texRender(node->tex[j], GL_TEXTURE3, j, textureLocs[j]);
					break;
				case 4:
					texRender(node->tex[j], GL_TEXTURE4, j, textureLocs[j]);
					break;
				case 5:
					texRender(node->tex[j], GL_TEXTURE5, j, textureLocs[j]);
					break;
				case 6:
					texRender(node->tex[j], GL_TEXTURE6, j, textureLocs[j]);
					break;
				case 7:
					texRender(node->tex[j], GL_TEXTURE7, j, textureLocs[j]);
					break;
			}
		}
		...
		/* Unrenders the node's textures */
		for (int k = 0; k < node->texNum; k++) {
			switch(k) {
				case 0:
					texUnrender(node->tex[k], GL_TEXTURE0);
					break;
				case 1:
					texUnrender(node->tex[k], GL_TEXTURE1);
					break;
				case 2:
					texUnrender(node->tex[k], GL_TEXTURE2);
					break;
				case 3:
					texUnrender(node->tex[k], GL_TEXTURE3);
					break;
				case 4:
					texUnrender(node->tex[k], GL_TEXTURE4);
					break;
				case 5:
					texUnrender(node->tex[k], GL_TEXTURE5);
					break;
				case 6:
					texUnrender(node->tex[k], GL_TEXTURE6);
					break;
				case 7:
					texUnrender(node->tex[k], GL_TEXTURE7);
					break;
			}
		}
