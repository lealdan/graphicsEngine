...
void camRender(camCamera *cam, GLint viewingLoc) {
	...
	mat44InverseIsometry(cam->rotation, cam->translation, tmp);
	if (cam->projectionType == camORTHOGRAPHIC) {
		...
		mat444Multiply(proj, tmp, viewing);
	}
	else if (cam->projectionType == camPERSPECTIVE) {
		mat44Perspective(cam->projection, proj);
		mat444Multiply(proj, tmp, viewing);
	}

	mat44OpenGL(viewing, openGL);
	glUniformMatrix4fv(viewingLoc, 1, GL_FALSE, (GLfloat *)openGL);
}
