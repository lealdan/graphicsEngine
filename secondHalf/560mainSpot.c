...
#define lightNUM 1
#define spotANGLE 18.0
#define lightPHI 5.0 * M_PI / 4.0
#define lightTHETA 0.3
...
int initializeShaderProgram(void) {
	...
	GLchar fragmentCode[] = "\
	    ...
	    void main() {\
			...
			float cosLam = dot(lightDir, -litDir);\
	...
	if (program != 0) {
		...
		lightLocs[3] = glGetUniformLocation(program, "lightDir");
		lightLocs[4] = glGetUniformLocation(program, "spotHalfAngle");
	}
	return (program == 0);
}
...
void render(void) {
	...
	lightShineFrom(lights[0], lightTrans, lightPHI, lightTHETA);
	lightSetColor(lights[0], lightColor);
	lightSetAttenuation(lights[0], lightAtt);
	lightSetSpotAngle(lights[0], spotANGLE);
	lightSetType(lights[0], lightSPOT);
	lightRender(lights[0], lightLocs[0], lightLocs[1], lightLocs[2], lightLocs[3], lightLocs[4]);
